...
for (( i=0; i<${#ARCHS[@]}; i++ )); do
	ARCH=${ARCHS[$i]}
	export CFLAGS="-arch $ARCH -pipe -Os -gdwarf-2 -isysroot $XCODE/Platforms/${PLATFORMS[$i]}.platform/Developer/SDKs/${SDK[$i]}.sdk -miphoneos-version-min=${IPHONEOS_DEPLOYMENT_TARGET} -Werror=partial-availability"
...
cd "$DESTDIR"
mkdir $DESTDIR/lib
lipo -create -output $DESTDIR/lib/libcurl.a libcurl-*.a
rm libcurl-*.a
